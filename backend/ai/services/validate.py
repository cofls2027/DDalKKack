...
async def validate_receipt(ocr_data: dict, card_type: str, company_id: int, position: str = None, headcount: int = 1) -> dict:
        ...
        if policy.get("category") == "금지품목" and policy.get("card_type") == card_type:
            ...
            for word in banned_keywords:
                if word in target:
                    return {
                        "status": "rejected",
                        "reason": f"{rule.get('rule_name')}: '{word}' 포함"
                    }
            break
    ...
    if receipt_time and allowed_time_from:
        if receipt_time < allowed_time_from:
            return {
                "status": "rejected",
                "reason": f"{applicable_rule.get('rule_name')}: {allowed_time_from} 이후만 허용 (영수증 시간: {receipt_time})"
            }
    ...
    if max_amount and per_person > max_amount:
        return {
            "status": "rejected",
            "reason": f"{applicable_rule.get('rule_name')}: 1인당 {per_person:,.0f}원 (한도: {max_amount:,}원, {headcount}명 기준)"
        }
    ...
    if approval_required:
        return {
            "status": "pending",
            "reason": f"{applicable_rule.get('rule_name')}: 관리자 승인 필요"
        }
